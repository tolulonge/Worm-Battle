...
public class Wormy : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(WormyManager.singleton.shouldRotate)
        RotateGun();
        Debug.Log(WormyManager.singleton.shouldRotate);


        if (WormyManager.singleton.horizontalInput == 0)
        {
            ...
            if (WormyManager.singleton.shouldShoot)
            {
        ...
        else
        {
            ...
            transform.position += Vector3.right *
                                WormyManager.singleton.horizontalInput *
                                Time.deltaTime *
                                wormySpeed;            
             ren.flipX = WormyManager.singleton.horizontalInput > 0;
        }
